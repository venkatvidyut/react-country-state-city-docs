...
:::info
The `GetCity` function in react-country-state-city fetches a list of [**city**](/latest/types/city) objects based on given country and state id's. The async function returns an array of [**city**](/latest/types/city) objects, each containing details such as name, latitude and longitude.
:::
...
Returns a [**city**](/latest/types/city) object lists.
